...
def user_add(request):
    ...
    user_role = {'SU': u'超级管理员', 'GU':u'组管理员', 'CU': u'普通用户', 'AU':u'审计用户'}
        ...
        try:
            ...
            try:
                ...
                user = db_add_user(username=username, name=name,
                                   ...
                                   groups=groups, productLine=productLine, admin_groups=admin_groups,
...
def user_list(request):
    ...
    if login_user.role == 'GU':
        users_list = User.objects.all().filter(productLine=login_user.productLine).order_by('username')
    else:
