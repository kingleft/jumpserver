...
from jproductline.models import ProductLine
...
def perm_rule_list(request):
    ...
    if user_perm == 2:  # 获取所有系统角色
        ...
        rules_list = PermRule.objects.all().filter(productLine=login_user.productLine)
...
def perm_rule_add(request):
    ...
    assets = Asset.objects.all().filter(productLine=login_user.productLine)
    #asset_groups = AssetGroup.objects.all()
    roles = PermRole.objects.all().filter(productLine=login_user.productLine)
    ...
    if request.method == 'POST':
        ...
        try:
            rule = get_object(PermRule, name=rule_name, productLine=productLine)
            ...
            rule = PermRule(name=rule_name, productLine=productLine, comment=rule_comment)
            rule.save()
            rule.user = users_obj
...
def perm_role_list(request):
    ...
    if user_perm == 2: # 获取所有系统角色
        ...
        login_user = get_object(User, username=username)
        roles_list = PermRole.objects.all().filter(productLine=login_user.productLine)
...
def perm_role_add(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            role = PermRole(name=name, productLine=productLine, comment=comment, password=encrypt_pass, key_path=key_path)
            role.save()
            role.sudo = sudos_obj
            msg = u"添加系统用户: %s" % name
...
def perm_role_edit(request):
    ...
    if user_perm == 2:
        ...
        login_user = get_object(User, username=username)
        sudo_all = PermSudo.objects.all().filter(productLine=login_user.productLine)
    if request.method == "GET":
...
def perm_sudo_list(request):
    ...
    if user_perm == 2:
        ...
        login_user = get_object(User, username=username)
        sudos_list = PermSudo.objects.all().filter(productLine=login_user.productLine)
    ...
    try:
        if request.method == "POST":
            ...
            if get_object(PermSudo, name=name, productLine=productLine):
                ...
                sudo = PermSudo(name=name.strip(), comment=comment, productLine=productLine, commands=commands)
                sudo.save()
