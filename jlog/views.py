...
def log_list(request, offset):
    ...
    if offset == 'online':
        ...
        if user_perm == 1:
            posts = Log.objects.filter(is_finished=False, host__in=host_names, user__in=user_names).order_by('-start_time')
        else:
            posts = Log.objects.filter(is_finished=False).order_by('-start_time')
        if keyword:
        ...
        if user_perm == 1:
            posts = ExecLog.objects.all().filter(host__in=host_names, user__in=user_names).order_by('-id')
        else:
        ...
        if user_perm == 1:
            posts = FileLog.objects.all().filter(host__in=host_names, user__in=user_names).order_by('-id')
        else:
        ...
        if user_perm == 1:
            posts = Log.objects.filter(is_finished=True, host__in=host_names, user__in=user_names).order_by('-start_time')
        else:
